...
- **Host your own infrastructure.** Any device can serve the mesh: a relay (roster-gated routing), a signaling relay (a NIP-01 server the built-in driver speaks to unchanged — drop public Nostr entirely), and STUN / TURN servers (RFC 5389 / 5766, via the webrtc-rs stack). Toggle them from the GUI (Settings → Services), the CLI (`myownmesh ctl services …`), or `config.json`; hosts advertise their roles + endpoints to peers so the fleet self-discovers them. This is what makes a **fully internet-isolated network** trivial — no Google STUN, no Cloudflare TURN, no public relay. See [`docs/SERVICES.md`](docs/SERVICES.md).
