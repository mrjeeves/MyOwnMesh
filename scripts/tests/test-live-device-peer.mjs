...
test("listener registration is provisional and retired member relay actions refuse", () => {
...
test("retired member relay and route trace actions cannot reach the real peer session RPC", async () => {
  for (const action of ["relay_echo_run", "relay_echo_listen", "route_trace_listen"]) {
    const writer = memoryWriter();
    let rpcCalls = 0;
    const { session, child } = createPeerSessionTestHarness({
      writer, rpc: async () => { rpcCalls++; return { ok: true }; },
    });
    try {
      const record = await session.execute({ id: action, action });
      assert.equal(record.type, "command_result");
      assert.equal(record.status, "failed");
      assert.deepEqual(record.error, {
        name: "ControllerError", message: `unsupported action ${action}`, kind: "failed",
      });
      assert.equal(rpcCalls, 0);
      child.exitCode = 0;
      await assert.rejects(session.close(), error => error?.kind === "failed");
      assert.equal(writer.closed, true);
...
test("only clean complete endpoint payload summaries pass", () => {
...
test("listener lifetime terminal summary and cleanup determine final status", () => {
  ...
  assert.equal(
    classifyLifetimeResult(
      "echo_listen",
      cleanPayload({ action: "echo_listen" }),
...
async function withEventHub(control) {
  const root = await mkdtemp(path.join(os.tmpdir(), "myownmesh-channel-events-"));
  const endpoint = process.platform === "win32"
  ...
  // This test fence itself makes no additional channel_subscribe RPC.
...
test("generic EventHub keeps direct channel dispatch and ignores unrelated diagnostics", { timeout: 5_000 }, async () => {
  await withEventHub(async ({ hub, pump, rpcCalls, subscriptions }) => {
    const received = [];
    const unsubscribe = await hub.subscribe("direct-network", "direct-channel", event => received.push(event));
    const expected = { kind: "channel_inbound", network: "direct-network",
      channel: "direct-channel", from: "endpoint", payload: { body: "test" } };
    await pump([{ kind: "event", event: { event_kind: "diag", category: "unrelated" } },
      { kind: "lagged", skipped: 9 }, { ...expected, network: "other" }, expected]);
    await unsubscribe();
    assert.deepEqual(received, [expected]);
    assert.equal(hub.failure, null);
    assert.equal(subscriptions.length, 1);
    assert.deepEqual(rpcCalls.map(request => request.op), ["channel_subscribe", "channel_unsubscribe"]);
  });
});
