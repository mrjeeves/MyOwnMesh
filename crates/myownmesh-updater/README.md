...
Self-update for the `myownmesh` binary.
...
3. SHA-256 verified against sidecar `.sha256`.
...
Package-manager installs (brew, apt, rpm, MSI, choco) are detected;
self-update defers to the OS package manager.

See `RELEASE.md` at the repo root for the publishing side.
