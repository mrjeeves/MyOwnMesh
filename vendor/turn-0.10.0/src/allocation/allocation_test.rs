...
async fn test_has_permission() -> Result<()> {
    ...
    a.try_add_permission(p1).await?;
    a.try_add_permission(p2).await?;
    a.try_add_permission(p3).await?;
...
async fn test_allocation_close() -> Result<()> {
    ...
    // add permission
    a.try_add_permission(Permission::new(addr)).await?;
