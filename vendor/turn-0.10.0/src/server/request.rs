...
use crate::proto::*;
use crate::resource::{ResourceAdmission, ResourceCharge, ResourceKind};
...
pub struct Request {
    ...
    pub nonces: Arc<Mutex<HashMap<String, (Instant, Box<dyn crate::resource::ResourceLease>)>>>,
    pub resource_admission: Option<Arc<dyn ResourceAdmission>>,
...
impl Request {
    pub fn new(
        ...
        Request {
            ...
            resource_admission: {
                #[cfg(test)]
                {
                    Some(Arc::new(crate::resource::UnboundedTestAdmission))
                }
                #[cfg(not(test))]
                {
                    None
    ...
    pub(crate) async fn authenticate_request(
        ...
        let to_be_deleted = {
            ...
            let to_be_deleted = if let Some((nonce_creation_time, _)) = nonces.get(&nonce_attr.text)
            {
                Instant::now()
    ...
    pub(crate) async fn handle_allocate_request(&mut self, m: &Message) -> Result<()> {
        ...
        let mut consumes_reservation = false;
        let mut creates_reservation = false;
        ...
        if reservation_token_attr_result.is_ok() {
            reservation_token = String::from_utf8(reservation_token_attr.0.clone())
                .map_err(|_| Error::ErrResourceAdmission)?;
            if self
                .allocation_manager
                .get_reservation(&reservation_token)
                .await
                .is_none()
            {
        ...
        if consumes_reservation {
            requested_port = self
                .allocation_manager
                .take_reservation(&reservation_token)
                .await
                .ok_or(Error::ErrResourceAdmission)?;
        }
        let a = match self
        ...
        if let Some(a) = a {
...

            {
                for attr in &m.attributes.0 {
                    ...
                    a.try_add_permission(Permission::new(SocketAddr::new(
                        peer_address.ip,
                        peer_address.port,
                    )))
                    .await?;
