...
pub enum Net {
    VNet(Arc<Mutex<VNet>>),
    Ifs(Vec<Interface>),
    /// Real interfaces with an opt-in socket configurator, run before use.
    IfsWithUdpConfig(Vec<Interface>, UdpSocketConfig),
}

pub type UdpSocketConfig = Arc<dyn Fn(&UdpSocket) -> std::io::Result<()> + Send + Sync>;

impl Net {
    /// Configure each real UDP socket before exposing it to ICE/STUN/TURN.
    /// Virtual networks and callers that do not opt in retain their behavior.
    pub fn with_udp_socket_config(self, configure: UdpSocketConfig) -> Self {
        match self {
            Net::Ifs(ifs) | Net::IfsWithUdpConfig(ifs, _) => Net::IfsWithUdpConfig(ifs, configure),
            virtual_net => virtual_net,
        }
    }

    async fn bind_real_udp(&self, addr: SocketAddr) -> Result<UdpSocket> {
        let socket = UdpSocket::bind(addr).await?;
        if let Net::IfsWithUdpConfig(_, configure) = self {
            configure(&socket)?;
        }
        Ok(socket)
    }
    ...
    pub async fn get_interfaces(&self) -> Vec<Interface> {
        match self {
            ...
            // Configured real networks may be shared by a long-lived API.
            // Refresh interfaces for each gather rather than freezing the
            // machine's adapters at transport startup.
            Net::IfsWithUdpConfig(_, _) => match Net::new(None) {
                Net::Ifs(ifs) => ifs,
                _ => unreachable!("real network constructor"),
            },
        }
    }
    ...
    pub async fn get_interface(&self, ifc_name: &str) -> Option<Interface> {
        match self {
            ...
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => self
                .get_interfaces()
                .await
                .into_iter()
                .find(|ifc| ifc.name == ifc_name),
        }
    }
    ...
    pub fn is_virtual(&self) -> bool {
        match self {
            Net::VNet(_) => true,
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => false,
        }
    }
    ...
    pub async fn resolve_addr(&self, use_ipv4: bool, address: &str) -> Result<SocketAddr> {
        match self {
            ...
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => {
                Ok(conn::lookup_host(use_ipv4, address).await?)
            }
        }
    }
    ...
    pub async fn bind(&self, addr: SocketAddr) -> Result<Arc<dyn Conn + Send + Sync>> {
        match self {
            ...
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => {
                Ok(Arc::new(self.bind_real_udp(addr).await?))
            }
        }
    }
    ...
    pub async fn dail(
        ...
        match self {
            ...
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => {
                ...
                let conn = self.bind_real_udp(local_addr).await?;
    ...
    pub fn get_nic(&self) -> Result<Arc<Mutex<dyn Nic + Send + Sync>>> {
        match self {
            ...
            Net::Ifs(_) | Net::IfsWithUdpConfig(_, _) => Err(Error::ErrVnetDisabled),
        }
    }
}
